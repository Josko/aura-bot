...
private:
  std::vector<uint_fast8_t> m_ClientToken;         // set in constructor
  std::vector<uint_fast8_t> m_LogonType;           // set in RECEIVE_SID_AUTH_INFO
  std::vector<uint_fast8_t> m_ServerToken;         // set in RECEIVE_SID_AUTH_INFO
  std::vector<uint_fast8_t> m_MPQFileTime;         // set in RECEIVE_SID_AUTH_INFO
  std::vector<uint_fast8_t> m_IX86VerFileName;     // set in RECEIVE_SID_AUTH_INFO
  std::vector<uint_fast8_t> m_ValueStringFormula;  // set in RECEIVE_SID_AUTH_INFO
  std::vector<uint_fast8_t> m_KeyState;            // set in RECEIVE_SID_AUTH_CHECK
  std::vector<uint_fast8_t> m_KeyStateDescription; // set in RECEIVE_SID_AUTH_CHECK
  std::vector<uint_fast8_t> m_Salt;                // set in RECEIVE_SID_AUTH_ACCOUNTLOGON
  std::vector<uint_fast8_t> m_ServerPublicKey;     // set in RECEIVE_SID_AUTH_ACCOUNTLOGON
  std::vector<uint_fast8_t> m_UniqueName;          // set in RECEIVE_SID_ENTERCHAT

public:
  CBNETProtocol();
  ~CBNETProtocol();

  inline std::vector<uint_fast8_t> GetClientToken() const { return m_ClientToken; }
  inline std::vector<uint_fast8_t> GetLogonType() const { return m_LogonType; }
  ...
  bool RECEIVE_SID_NULL(const std::vector<uint_fast8_t>& data);
  CIncomingGameHost* RECEIVE_SID_GETADVLISTEX(const std::vector<uint_fast8_t>& data);
  bool RECEIVE_SID_ENTERCHAT(const std::vector<uint_fast8_t>& data);
  CIncomingChatEvent* RECEIVE_SID_CHATEVENT(const std::vector<uint_fast8_t>& data);
  bool RECEIVE_SID_CHECKAD(const std::vector<uint_fast8_t>& data);
  bool RECEIVE_SID_STARTADVEX3(const std::vector<uint_fast8_t>& data);
  std::vector<uint_fast8_t> RECEIVE_SID_PING(const std::vector<uint_fast8_t>& data);
  bool RECEIVE_SID_AUTH_INFO(const std::vector<uint_fast8_t>& data);
  bool RECEIVE_SID_AUTH_CHECK(const std::vector<uint_fast8_t>& data);
  bool RECEIVE_SID_AUTH_ACCOUNTLOGON(const std::vector<uint_fast8_t>& data);
  bool RECEIVE_SID_AUTH_ACCOUNTLOGONPROOF(const std::vector<uint_fast8_t>& data);
  std::vector<std::string> RECEIVE_SID_FRIENDLIST(const std::vector<uint_fast8_t>& data);
  std::vector<std::string> RECEIVE_SID_CLANMEMBERLIST(const std::vector<uint_fast8_t>& data);

  // send functions

  std::vector<uint_fast8_t> SEND_PROTOCOL_INITIALIZE_SELECTOR();
  std::vector<uint_fast8_t> SEND_SID_NULL();
  std::vector<uint_fast8_t> SEND_SID_STOPADV();
  std::vector<uint_fast8_t> SEND_SID_GETADVLISTEX(const std::string& gameName);
  std::vector<uint_fast8_t> SEND_SID_ENTERCHAT();
  ...
  std::vector<uint_fast8_t> SEND_SID_FRIENDLIST();
  std::vector<uint_fast8_t> SEND_SID_CLANMEMBERLIST();
