...
public:
  ...
  inline std::vector<uint_fast8_t> GetExternalIP() const { return m_Socket->GetIP(); }
  ...
  void Send(const std::vector<uint_fast8_t>& data) const;
};
...
private:
  std::vector<uint_fast8_t>             m_InternalIP;                   // the player's internal IP address as reported by the player when connecting
  std::vector<uint_fast32_t>            m_Pings;                        // store the last few (10) pings received so we can take an average
  std::queue<uint_fast32_t>             m_CheckSums;                    // the last few checksums the player has sent (for detecting desyncs)
  std::queue<std::vector<uint_fast8_t>> m_GProxyBuffer;                 // buffer with data used with GProxy++
  ...
  uint_fast32_t                         m_TotalPacketsSent;             // the total number of packets sent to the player
  uint_fast32_t                         m_TotalPacketsReceived;         // the total number of packets received from the player
  uint_fast32_t                         m_LeftCode;                     // the code to be sent in W3GS_PLAYERLEAVE_OTHERS for why this player left the game
  uint_fast32_t                         m_SyncCounter;                  // the number of keepalive packets received from this player
  int_fast64_t                          m_JoinTime;                     // GetTime when the player joined the game (used to delay sending the /whois a few seconds to allow for some lag)
  uint_fast32_t                         m_LastMapPartSent;              // the last mappart sent to the player (for sending more than one part at a time)
  uint_fast32_t                         m_LastMapPartAcked;             // the last mappart acknowledged by the player
  int_fast64_t                          m_StartedDownloadingTicks;      // GetTicks when the player started downloading the map
  int_fast64_t                          m_FinishedDownloadingTime;      // GetTime when the player finished downloading the map
  int_fast64_t                          m_FinishedLoadingTicks;         // GetTicks when the player finished loading the game
  int_fast64_t                          m_StartedLaggingTicks;          // GetTicks when the player started laggin
  int_fast64_t                          m_LastGProxyWaitNoticeSentTime; // GetTime when the last disconnection notice has been sent when using GProxy++
  uint_fast32_t                         m_GProxyReconnectKey;           // the GProxy++ reconnect key
  int_fast64_t                          m_LastGProxyAckTime;            // GetTime when we last acknowledged GProxy++ packet
  uint_fast8_t                          m_PID;                          // the player's PID
...
public:
  CGamePlayer(CPotentialPlayer* potential, uint_fast8_t nPID, std::string nJoinedRealm, std::string nName, std::vector<uint_fast8_t> nInternalIP, bool nReserved);
  ~CGamePlayer();

  uint_fast32_t GetPing(bool LCPing) const;
  inline CTCPSocket*                GetSocket() const { return m_Socket; }
  inline std::vector<uint_fast8_t>  GetExternalIP() const { return m_Socket->GetIP(); }
  inline std::string                GetExternalIPString() const { return m_Socket->GetIPString(); }
  inline bool                       GetDeleteMe() const { return m_DeleteMe; }
  ...
  inline uint_fast32_t              GetNumPings() const { return m_Pings.size(); }
  inline uint_fast32_t              GetNumCheckSums() const { return m_CheckSums.size(); }
  inline std::queue<uint_fast32_t>* GetCheckSums() { return &m_CheckSums; }
  ...
  inline uint_fast32_t              GetLeftCode() const { return m_LeftCode; }
  inline uint_fast32_t              GetSyncCounter() const { return m_SyncCounter; }
  inline int_fast64_t               GetJoinTime() const { return m_JoinTime; }
  inline uint_fast32_t              GetLastMapPartSent() const { return m_LastMapPartSent; }
  inline uint_fast32_t              GetLastMapPartAcked() const { return m_LastMapPartAcked; }
  inline int_fast64_t               GetStartedDownloadingTicks() const { return m_StartedDownloadingTicks; }
  inline int_fast64_t               GetFinishedDownloadingTime() const { return m_FinishedDownloadingTime; }
  inline int_fast64_t               GetFinishedLoadingTicks() const { return m_FinishedLoadingTicks; }
  inline int_fast64_t               GetStartedLaggingTicks() const { return m_StartedLaggingTicks; }
  inline int_fast64_t               GetLastGProxyWaitNoticeSentTime() const { return m_LastGProxyWaitNoticeSentTime; }
  inline uint_fast32_t              GetGProxyReconnectKey() const { return m_GProxyReconnectKey; }
  ...
  inline void SetLeftCode(uint_fast32_t nLeftCode) { m_LeftCode = nLeftCode; }
  inline void SetSyncCounter(uint_fast32_t nSyncCounter) { m_SyncCounter = nSyncCounter; }
  inline void SetLastMapPartSent(uint_fast32_t nLastMapPartSent) { m_LastMapPartSent = nLastMapPartSent; }
  inline void SetLastMapPartAcked(uint_fast32_t nLastMapPartAcked) { m_LastMapPartAcked = nLastMapPartAcked; }
  inline void SetStartedDownloadingTicks(uint_fast64_t nStartedDownloadingTicks) { m_StartedDownloadingTicks = nStartedDownloadingTicks; }
  inline void SetFinishedDownloadingTime(uint_fast64_t nFinishedDownloadingTime) { m_FinishedDownloadingTime = nFinishedDownloadingTime; }
  inline void SetStartedLaggingTicks(uint_fast64_t nStartedLaggingTicks) { m_StartedLaggingTicks = nStartedLaggingTicks; }
  ...
  inline void SetLastGProxyWaitNoticeSentTime(uint_fast64_t nLastGProxyWaitNoticeSentTime) { m_LastGProxyWaitNoticeSentTime = nLastGProxyWaitNoticeSentTime; }
  ...
  void Send(const std::vector<uint_fast8_t>& data);
  void EventGProxyReconnect(CTCPSocket* NewSocket, uint_fast32_t LastPacket);
};
