...
public:
  ...
  BYTEARRAY SEND_GPSC_RECONNECT(uint8_t PID, uint32_t reconnectKey, uint32_t lastPacket);
  BYTEARRAY SEND_GPSC_ACK(uint32_t lastPacket);
  BYTEARRAY SEND_GPSS_INIT(uint16_t reconnectPort, uint8_t PID, uint32_t reconnectKey, uint8_t numEmptyActions);
