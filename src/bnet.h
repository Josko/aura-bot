...
private:
  ...
  int_fast64_t                          m_LastDisconnectedTime;      // GetTime when we were last disconnected from battle.net
  int_fast64_t                          m_LastConnectionAttemptTime; // GetTime when we last attempted to connect to battle.net
  int_fast64_t                          m_LastNullTime;              // GetTime when the last null packet was sent for detecting disconnects
  int_fast64_t                          m_LastOutPacketTicks;        // GetTicks when the last packet was sent for the m_OutPackets queue
  int_fast64_t                          m_LastAdminRefreshTime;      // GetTime when the admin list was last refreshed from the database
  int_fast64_t                          m_LastBanRefreshTime;        // GetTime when the ban list was last refreshed from the database
  int_fast64_t                          m_ReconnectDelay;            // interval between two consecutive connect attempts
  uint_fast32_t                         m_LastOutPacketSize;         // byte size of the last packet we sent from the m_OutPackets queue
  uint_fast32_t                         m_LocaleID;                  // see: http://msdn.microsoft.com/en-us/library/0h88fahh%28VS.85%29.aspx
  uint_fast32_t                         m_HostCounterID;             // the host counter ID to identify players from this realm
...
public:
  CBNET(CAura* nAura, std::string nServer, std::string nServerAlias, std::string nCDKeyROC, std::string nCDKeyTFT, std::string nCountryAbbrev, std::string nCountry, uint_fast32_t nLocaleID, std::string nUserName, std::string nUserPassword, std::string nFirstChannel, char nCommandTrigger, uint_fast8_t nWar3Version, std::vector<uint_fast8_t> nEXEVersion, std::vector<uint_fast8_t> nEXEVersionHash, std::string nPasswordHashType, uint_fast32_t nHostCounterID);
  ~CBNET();
  CBNET(CBNET&) = delete;
  ...
  inline uint_fast32_t             GetHostCounterID() const { return m_HostCounterID; }
  inline bool                      GetLoggedIn() const { return m_LoggedIn; }
  inline bool                      GetInChat() const { return m_InChat; }
  inline uint_fast32_t             GetOutPacketsQueued() const { return m_OutPackets.size(); }
  ...
  void QueueGameCreate(uint_fast8_t state, const std::string& gameName, CMap* map, uint_fast32_t hostCounter);
  void QueueGameRefresh(uint_fast8_t state, const std::string& gameName, CMap* map, uint_fast32_t hostCounter);
