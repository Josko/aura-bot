...
public:
  ...
  uint_fast32_t            m_ReconnectWaitTime;  // config value: the maximum number of minutes to wait for a GProxy++ reliable reconnect
  uint_fast32_t            m_MaxGames;           // config value: maximum number of games in progress
  uint_fast32_t            m_HostCounter;        // the current host counter (a unique number to identify a game, incremented each time a game is created)
  uint_fast32_t            m_AllowDownloads;     // config value: allow map downloads or not
  uint_fast32_t            m_MaxDownloaders;     // config value: maximum number of map downloaders at the same time
  uint_fast32_t            m_MaxDownloadSpeed;   // config value: maximum total map download speed in KB/sec
  uint_fast32_t            m_AutoKickPing;       // config value: auto kick players with ping higher than this
  uint_fast32_t            m_LobbyTimeLimit;     // config value: auto close the game lobby after this many minutes without any reserved players
  uint_fast32_t            m_Latency;            // config value: the latency (by default)
  uint_fast32_t            m_SyncLimit;          // config value: the maximum number of packets a player can fall out of sync before starting the lag screen (by default)
  uint_fast32_t            m_VoteKickPercentage; // config value: percentage of players required to vote yes for a votekick to pass
  uint_fast16_t            m_HostPort;           // config value: the port to host games on
  uint_fast16_t            m_ReconnectPort;      // config value: the port to listen for GProxy++ reliable reconnects on
  uint_fast8_t             m_LANWar3Version;     // config value: LAN warcraft 3 version
  int_fast32_t             m_CommandTrigger;     // config value: the command trigger inside games
