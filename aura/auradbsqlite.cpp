...
CAuraDBSQLite :: CAuraDBSQLite( CConfig *CFG ) : CAuraDB( CFG )
{
...
bool CAuraDBSQLite :: Begin( )
{
	return m_DB->Exec( "BEGIN TRANSACTION" ) == SQLITE_OK;
}

bool CAuraDBSQLite :: Commit( )
{
	return m_DB->Exec( "COMMIT TRANSACTION" ) == SQLITE_OK;
}

uint32_t CAuraDBSQLite :: AdminCount( const string &server )
{
	uint32_t Count = 0;
	sqlite3_stmt *Statement;
...
bool CAuraDBSQLite :: AdminCheck( const string &server, string user )
{
...
bool CAuraDBSQLite :: AdminAdd( const string &server, string user )
{
	transform( user.begin( ), user.end( ), user.begin( ), (int(*)(int))tolower );
	bool Success = false;
	sqlite3_stmt *Statement;
...
bool CAuraDBSQLite :: AdminRemove( const string &server, string user )
{
	transform( user.begin( ), user.end( ), user.begin( ), (int(*)(int))tolower );
	bool Success = false;
	sqlite3_stmt *Statement;
...
vector<string> CAuraDBSQLite :: AdminList( const string &server )
{
...
uint32_t CAuraDBSQLite :: BanCount( const string &server )
{
	uint32_t Count = 0;
	sqlite3_stmt *Statement;
...
CDBBan *CAuraDBSQLite :: BanCheck( const string &server, string user, const string &ip )
{
	transform( user.begin( ), user.end( ), user.begin( ), (int(*)(int))tolower );
	CDBBan *Ban = NULL;
...
bool CAuraDBSQLite :: BanAdd( const string &server, string user, const string &ip, const string &gamename, string admin, const string &reason )
{
	transform( user.begin( ), user.end( ), user.begin( ), (int(*)(int))tolower );
	bool Success = false;
	sqlite3_stmt *Statement;
...
bool CAuraDBSQLite :: BanRemove( const string &server, string user )
{
	transform( user.begin( ), user.end( ), user.begin( ), (int(*)(int))tolower );
	bool Success = false;
	sqlite3_stmt *Statement;
...
bool CAuraDBSQLite :: BanRemove( string user )
{
	transform( user.begin( ), user.end( ), user.begin( ), (int(*)(int))tolower );
	bool Success = false;
	sqlite3_stmt *Statement;
...
vector<CDBBan *> CAuraDBSQLite :: BanList( const string &server )
{
...
uint32_t CAuraDBSQLite :: GameAdd( const string &server, const string &map, const string &gamename, const string &ownername, uint32_t duration, uint32_t gamestate, const string &creatorname, const string &creatorserver )
{
	uint32_t RowID = 0;
	sqlite3_stmt *Statement;
...
uint32_t CAuraDBSQLite :: GamePlayerAdd( uint32_t gameid, string name, const string &ip, uint32_t spoofed, const string &spoofedrealm, uint32_t reserved, uint32_t loadingtime, uint32_t left, const string &leftreason, uint32_t team, uint32_t colour )
{
...
uint32_t CAuraDBSQLite :: GamePlayerCount( string name )
{
	transform( name.begin( ), name.end( ), name.begin( ), (int(*)(int))tolower );
	uint32_t Count = 0;
	sqlite3_stmt *Statement;
...
CDBGamePlayerSummary *CAuraDBSQLite :: GamePlayerSummaryCheck( string name )
{
...
uint32_t CAuraDBSQLite :: DotAGameAdd( uint32_t gameid, uint32_t winner, uint32_t min, uint32_t sec )
{
	uint32_t RowID = 0;
	sqlite3_stmt *Statement;
...
uint32_t CAuraDBSQLite :: DotAPlayerAdd( uint32_t gameid, uint32_t colour, uint32_t kills, uint32_t deaths, uint32_t creepkills, uint32_t creepdenies, uint32_t assists, uint32_t gold, uint32_t neutralkills, const string &item1, const string &item2, const string &item3, const string &item4, const string &item5, const string &item6, const string &hero, uint32_t newcolour, uint32_t towerkills, uint32_t raxkills, uint32_t courierkills )
{
	uint32_t RowID = 0;
	sqlite3_stmt *Statement;
...
uint32_t CAuraDBSQLite :: DotAPlayerCount( string name )
{
	transform( name.begin( ), name.end( ), name.begin( ), (int(*)(int))tolower );
	uint32_t Count = 0;
	sqlite3_stmt *Statement;
...
CDBDotAPlayerSummary *CAuraDBSQLite :: DotAPlayerSummaryCheck( string name )
{
...
string CAuraDBSQLite :: FromCheck( uint32_t ip )
{
...
bool CAuraDBSQLite :: FromAdd( uint32_t ip1, uint32_t ip2, const string &country )
{
	// a big thank you to tjado for help with the iptocountry feature
